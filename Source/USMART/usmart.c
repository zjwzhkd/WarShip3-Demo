...
//2,增加USMART_ENTIM_SCAN宏定义,用于配置是否使用TIM2定时执行scan函数.
//V2.5 20110930
...
//其他的:TIM_x_IRQHandler和usmart_TimerInit,需要根据MCU特点自行修改.确保计数器计数频率为:10Khz即可.另外,定时器不要开启自动重装载功能!!

#define TIM_x                   TIM4
#define TIM_x_IRQn              TIM4_IRQn
#define TIM_x_IRQHandler        TIM4_IRQHandler
#define TIM_x_PreemptPriority   ( 15 )

#define RCC_TIM_CLK_ENABLE()    RCC_APB1PeriphClockCmd(RCC_APB1Periph_TIM4, ENABLE)
...
{
    TIM_ClearFlag(TIM_x,TIM_FLAG_Update);//清除中断标志位
    TIM_SetAutoreload(TIM_x,0XFFFF);//将重装载值设置到最大
    TIM_SetCounter(TIM_x,0);     //清空定时器的CNT
...
{
    if(TIM_GetFlagStatus(TIM_x,TIM_FLAG_Update)==SET)//在运行期间,产生了定时器溢出
    {
        usmart_dev.runtime+=0XFFFF;
    }
    usmart_dev.runtime+=TIM_GetCounter(TIM_x);
...
{
    if(TIM_GetITStatus(TIM_x,TIM_IT_Update)==SET)//溢出中断
    {
        usmart_dev.scan();  //执行usmart扫描
        TIM_SetCounter(TIM_x,0);     //清空定时器的CNT
        TIM_SetAutoreload(TIM_x,1000-1);//恢复原来的设置
    }
    TIM_ClearITPendingBit(TIM_x,TIM_IT_Update);  //清除中断标志位
}
//定时器初始化
void usmart_TimerInit(u16 arr,u16 psc)
{
    ...
    TIM_ITConfig( TIM_x, TIM_IT_Update|TIM_IT_Trigger, ENABLE );//TIM允许更新，触发中断

    //TIM_x中断分组配置
    cpu_NVIC_SetPriority(TIM_x_IRQn, TIM_x_PreemptPriority, 0);
    cpu_NVIC_EnableIRQ(TIM_x_IRQn);
...
{
#if USMART_ENTIMX_SCAN==1
    usmart_TimerInit(1000-1,(u32)sysclk*100-1);//分频,时钟为10K ,100ms中断一次,注意,计数频率必须为10Khz,以和runtime单位(0.1ms)同步.
#endif
...
void usmart_scan(void)
{
    u8 sta,len;
    u8 *buf;
    if(bsp_Debug_IsReceiveComplete())//串口接收完成？
    {
        len=bsp_Debug_GetReceiveLen();  //得到此次接收到的数据长度
        buf=bsp_Debug_GetReceiveBuffer();
        buf[len]='\0'; //在末尾加入结束符.
        sta=usmart_dev.cmd_rec(buf);//得到函数各个信息
        if(sta==0)usmart_dev.exe(); //执行函数
        else
        {
            len=usmart_sys_cmd_exe(buf);
